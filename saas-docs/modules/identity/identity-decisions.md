...
| Place order via QR | ✅ | — | **Fully anonymous — no name/phone/email captured at all.** Deliberately *not* the walk-in shape (§6.1) — no `business_customers` row is created for a QR order, so there's no repeat-visit recognition and no claim-to-account path (D6) for this specific interaction. Order is recorded with full details but has no CRM link; identified only by a per-order access token. See `guest-ordering-flow.md` for the full design and the trade-off this accepts. |
